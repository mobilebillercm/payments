...
use PhpAmqpLib\Connection\AMQPStreamConnection;
    ...
    $channel->exchange_declare(
        parse_ini_file("global-var-config.ini", true)['EXCHANGES']['PAYMENT_WITH_MOBILE_BILLER_ACCOUNT_FAILED_EXCHANGE'],
        'fanout',
        false,
        true,
        false);
    ...
    list($queue_name, ,) = $channel->queue_declare(
        parse_ini_file("global-var-config.ini", true)['QUEUES']['PAYMENT_PAYMENT_WITH_MOBILE_BILLER_ACCOUNT_FAILED_QUEUE'],
        false,
        true,
        false,
        false);
    ...
    echo " [*] Waiting for payement with mobile biller credit acount to be marked as failed to exit press CTRL+C\n";
    ...
    $callback = function ($msg) {
        ...
        try {
            ...
            $tokenUrl = parse_ini_file("global-var-config.ini", true)['URLS']['HOST_PAYMENTS'] . '/oauth/token';
        ...
        try {
            ...
            $url = parse_ini_file("global-var-config.ini", true)['URLS']['HOST_PAYMENTS'] . '/api/payments-from-mobile-biller-credit-account-failed';
